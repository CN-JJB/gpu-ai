...
def main():
    ...
    with tempfile.TemporaryDirectory() as td_raw:
        ...
        model_bytes = b"tiny-model-for-quality-identity-i27\n"
        model = td / "model.gguf"
        model.write_bytes(model_bytes)

        profile_bytes = b"tiny-profile-for-quality-identity-i27\n"
        ...
        assert "non-synthetic intake requires --quality-manifest" in out
        assert "QUALITY IDENTITY" in out
        assert "status=BLOCKED" in out
        assert "INTAKE: BLOCKED" in out

        out = run(base + ["--quality-manifest", str(quality_manifest)])
        assert "QUALITY IDENTITY" in out
        assert "status=PASS" in out
        assert "RAW IDENTITY: PASS" in out
        assert "INTAKE: READY" in out

        bad_quality = td / "bad-quality-identity.json"
        bad_obj = json.loads(quality_manifest.read_text(encoding="utf-8"))
        bad_obj["evaluation_args"] = "wrongxx"
        ...
        out = run(bad_base + ["--quality-manifest", str(bad_quality)], expect=2)
        assert "quality manifest evaluation_args != Experiment 61 manifest" in out
        assert "INTAKE: BLOCKED" in out

    print("QUALITY IDENTITY GATE SELFTEST: PASS")
    print("- non-synthetic intake requires a machine-readable quality identity manifest")
    print("- tokenizer/corpus/fixture/evaluation identity plus PACKET coverage pass")
    print("- semantic quality identity mismatch remains blocked after PACKET is recomputed")
